...
导师模式的 `config` 命令提供分步配置向导：可选择常用接口预设、新建或更新命名 profile、切换已有 profile，并在确认后生成 TOML 文件及立即启用。未显式传入 `--config` 时默认保存到统一数据目录的 `config.toml`，后续启动自动发现并加载；显式配置路径优先。手工编辑配置文件的方式继续保留。OpenAI 兼容服务允许填写服务根地址或 `/v1` 根地址，由客户端补全 Chat Completions 路径；Ollama 不发送不兼容的布尔 `reasoning` 扩展。HTTP 非成功响应应保留经过长度限制的服务端错误详情，便于排查配置问题。
...
所有诊断会话自动保存到 `sessions/auto/`，供顶层 `resume` 命令使用；`save`/`--save` 是独立的手动导出操作，保存到 `sessions/exported/`。Markdown 报告统一保存到 `reports/`，学习画像默认保存到 `learning/profile.json`。自动会话路径仅在启动导师会话时提示一次，`recheck` 静默更新同一文件；用户主动导出时仍输出完整路径。

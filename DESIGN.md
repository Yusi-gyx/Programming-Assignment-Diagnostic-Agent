...
导师模式的 `config` 命令提供分步配置向导：可选择常用接口预设、新建或更新命名 profile、切换已有 profile，并在确认后生成 TOML 文件及立即启用。配置统一保存在数据目录的 `config.toml`，每次诊断自动发现并加载；文件不存在时不启用模型。CLI 不再暴露独立的 `--config` 路径参数，手工编辑统一配置文件的方式继续保留。OpenAI 兼容服务允许填写服务根地址或 `/v1` 根地址，由客户端补全 Chat Completions 路径；Ollama 不发送不兼容的布尔 `reasoning` 扩展。HTTP 非成功响应应保留经过长度限制的服务端错误详情，便于排查配置问题。
...
测试失败本身及期望/实际输出由 Rust 确定。若配置了模型，系统将题目、代码和失败用例批量交给模型做语义分析，再由 Rust 把模型输出校验并限制到 `KnowledgePoint` 枚举；报告、Level 3 提示与学习画像使用该映射。未配置模型或映射失败时保留基础逻辑错误分类，不伪造知识点。
...
Level 1-2 由 Rust 确定性生成。Level 3-4 先保留 Rust 生成的知识点和修改方向，在存在模型配置时再进行教学增强：Level 3 解释已映射知识点并给出与当前作业不同的通用示例；Level 4 给出经典错误模式与通用改进写法。两级 prompt 必须明确禁止输出当前作业的完整答案。Level 5 使用“问题原因、修改步骤、关键代码片段、自检”的结构化输出。模型响应在展示前移除思维链标签和多余 Markdown 外层，并对多行内容进行缩进排版。模型调用不复用动态进度条，而使用开始、完成、失败静态标志。
...
所有诊断会话自动保存到 `sessions/auto/`，供顶层 `resume` 命令使用；`save`/`--save` 是独立的手动导出操作，保存到 `sessions/exported/`。导出目标重名时，交互终端必须询问是否覆盖；拒绝覆盖后继续要求新文件名并再次检测。非交互模式不得静默覆盖，而应提示用户更换名称。Markdown 报告统一保存到 `reports/`，学习画像默认保存到 `learning/profile.json`。自动会话路径仅在启动导师会话时提示一次，`recheck` 静默更新同一文件；用户主动导出时仍输出完整路径。

真实终端采用语义颜色：错误类型红色、知识点黄色、提示蓝色、成功结果绿色；非终端输出或设置 `NO_COLOR` 时保持纯文本。

为降低命令记忆成本，命令前的反斜杠可省略。真实终端默认进入导师模式；
`--no-interactive` 可用于 CI / 脚本。`--step` 在各阶段展示序号、即将执行的操作及用途，支持继续当前阶段、执行本轮剩余步骤、帮助和安全取消；`recheck` 开始新一轮逐步确认。逐步模式不叠加动态进度条，非交互终端则提示后按连续模式执行。
...
| 参数 | 说明 |
|------|------|
| `--tests <file>`（兼容 `--test`） | 加载 JSON 格式的输入/期望输出测试 |
